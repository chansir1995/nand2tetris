...
public:
	...
	void compileLet() {
		...
		eat(1, IDENTIFIER);
		if (!findId(cur)) {
			cout << "Syntax error : identifier \"" << cur << "\" must be declared before used in line " << to_string(t.getCurLine()) << endl;
			exit(-1);
		}
	...
	int compileTerm(int force) {
		...
		if (eat(0, INT_CONST)) {
			...
			if (eat(0, SYMBOL, "[")) { //varName[expression]
				if (type != "Array") {
					cout << "Syntax error : array identifier \"" << cur << "\" must be declared before used in line " << to_string(t.getCurLine()) << endl;
					exit(-1);
				}		
				writer.writePush(transIdentifier[kind], index);
				compileExpression(1);
			...
			/* subroutineCall */
				...
				writer.writePush("pointer", 0);
				nArgs = compileExpressionList();
				writer.writeCall(className + "." + str1, nArgs + 1); // 参数要不要包含this指针?
				eat(1, SYMBOL, ")");
			}
			/* just a varName */
			else{
